						...
						<section class="detail_section detail_info1">
							<dl>
								...
								<dd class="detail_price">
									<div>
										<span class="price">${item.price}<em class="won">원</em>
...
<form action="../cart/cartAdd.shop" name="fs">
 <input type="hidden" name="itemno" value="${item.itemno}">
         ...
         <ul class="prdtList" id="prdtList">
           ...
           <div class="add_prd_layer"></div>        
			...
			<div class="product_detail_box3 ">
				<div class="info_box1" id="info_box1">
					...
					<ul class="clearfix detail_tab" style="width:65%;">
						...
						<li><a href="#info_box2">상품 후기 (<span class="reviewCount">${ecount}</span>)
						</a></li>
			...
			<img alt="" src="../picture/${pic.pictureurl}" style="height: 1822px; width: 980px">
			</c:if>
			</c:forEach>
						</p>
					</section>
...
<c:if test="${ecount !=0}">
   ...
   <c:if test="${e.itemno eq param.itemno}">
   <tr class="group-center">
		<td class="align-center toggle_btn"><a href="#">${e.title}</a></td>
		<c:choose>
...
<tbody>
	<tr class="group-center">
<td><input type="hidden" name="itemno" value="${param.itemno}"></td>
<c:if test="${qavg==0}">
	<td colspan="5">작성된 상품 Q&amp;A가 없습니다.</td>
</c:if>
<c:if test="${qavg != 0}">
	<c:forEach items="${qlist}" var="q">
	<c:if test="${q.itemno eq param.itemno}">
		<tr class="group-center">
    ...
    <span class="ico_answer fl-l">답변</span> 
      <div class="answer_txt align-center">${q.answer}</div>
         ...
         <c:if test="${loginUser.userId == 'admin' }">
           <a href="../board/answer.shop?num=${q.num}&itemno=${q.itemno}" class="btn_mType1" style="align:right;"> 
